...
let rankBoard = {}
let rankBoardView = document.querySelector('.rankBoard');
...
function initTyping(socket) {
    ...
    if (charIndex < characters.length - 1 && timeLeft > 0) {
        ...
        socket.emit('message', { wpm: wpm, mistakes: mistakes })
    } else {
        clearInterval(timer);
        inpField.value = "";
    }
...
const makeSocketConnection = async () => {
    let username = await localStorage.getItem('username');
    const socket = io('https://typing.igaurav.dev', {
        auth: {
            username: username
        }
    });
    socket.on('connect', () => {
        ...
        inpField.addEventListener("input", () => { initTyping(socket) });
        tryAgainBtn.addEventListener("click", resetGame);
    });

    socket.on('message', (data) => {
        let { username } = data;
        rankBoard[username] = data;
        let records = Object.values(rankBoard);
        records.sort((a, b) => {
